...
func TestIo(t *testing.T) {
	...
	writteen, err := io.CopyN(dst, src, 58)
	if err != nil {
		fmt.Printf("error:%v\n", err)
	} else {
...
func excuteInfo(err error, f func()) {
	if err != nil {
		fmt.Printf("error:%v\n", err)
		return
	}
...
func TestIoFunc(t *testing.T) {
	...
	reader2 := io.LimitReader(reader1, 7) // 只读取7个字符长度给reader2对象
	buf2 := make([]byte, 10)
	for i := 0; i < 3; i++ {
		n, err = reader2.Read(buf2)
		excuteInfo(err, func() {
			fmt.Printf("read(%d):%q", n, buf2[:n])
		})
	}
	fmt.Println()
...
func TestIoFunc3(t *testing.T) {
	...
	reader2 := io.NewSectionReader(basicReader, 98, 98)
	...
	reader4 := io.MultiReader(basicReader)
